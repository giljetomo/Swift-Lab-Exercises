...
var meals: [String: Meal] = ["Breakfast": Meal(food: ["Bagel", "Orange Juice", "Egg Whites"], calories: 530),
                             "Lunch": Meal(food: ["a", "b", "c"], calories: 1)]

func logFood (_ food: String) -> Meal? {
    if let log = meals[food] {
        return log
    }
    return nil
}

if let meal = logFood("Lunch") {
    for a in meal.food {
        print(a)
    }
} else {
...
func checkLog () -> [String: Any] {
    if let a = UserDefaults.standard.dictionary(forKey: "mealLog") {
        return a
    }
    return [:]
}
